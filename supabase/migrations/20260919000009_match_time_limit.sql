-- Adds a match time limit (5 minutes, ~30s/question average — a
-- placeholder, not balanced) with two effects:
--   1. The frontend shows a countdown, adding real time pressure.
--   2. In PvP, if a player never finishes (disconnected, abandoned, closed
--      the tab), the OTHER player is no longer stuck waiting forever — once
--      the time limit has passed, scoring proceeds treating the
--      unfinished player's unanswered questions as incorrect, rather than
--      raising an exception. This is the "time limit as a product-safe
--      default so a match can't be stuck" case the plan flagged, and it's
--      a much simpler fix than building full Presence-based disconnect
--      detection — that remains a real gap (nothing detects a live
--      disconnect *before* the time limit elapses), but this bounds the
--      worst case to "wait out the clock" instead of "wait forever".
